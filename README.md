<p align="center">
  <img src="./docs/readme-hero.svg" alt="凤冠 Phoenix Crown — 汉字帘幕下的七顶凤冠交互光影可视化" width="100%" />
</p>

# 字符帘幕凤冠 · Phoenix Crown

汉字帘幕凤冠可视化站点。对标 [`aigc17/Chinese-PhoenixCrown`](https://github.com/aigc17/Chinese-PhoenixCrown) 的混合栈重构：打开网页，一顶凤冠悬于光影，**汉字帘幕沿冠形垂落、随风轻摆**，左右轻拨即切换七顶华美。

> 包管理用 **bun**（`bun.lock` 锁定），也可用 npm/pnpm。

## 技术栈

| 层 | 技术 | 说明 |
|---|---|---|
| 构建 | Vite `^8.1.4` · React `^19.2.0` · TypeScript `^7.0.2` | 纯客户端 SPA，无 SSR |
...
| 冠形拼接 | 采样凤冠 PNG alpha 轮廓 | 帘幕沿冠形垂挂，非直线 |
| 部署 | `@cloudflare/vite-plugin` `^1.44.0` + wrangler `^4.110.0` | `bun run build` 自动产出 `wrangler.json`，部署 Cloudflare |

**运行时依赖**：`react` · `react-dom` · `@chenglou/pretext`
**开发依赖**：`vite` · `@vitejs/plugin-react` · `typescript` · `@types/react` · `@types/react-dom` · `tailwindcss` · `@tailwindcss/vite` · `@cloudflare/vite-plugin` · `wrangler`
...
- **中 / 英文切换**：顶栏右侧 `中 / EN` 按钮。英文模式仅翻译 UI 标签（导航、按钮、aria），冠名与诗句保留中文（内容不译）。
  - 注：英文模式定位是 **UI 本地化**，不是全文翻译。冠名、诗句是文化内容，刻意保留中文原貌，不属于未完成的翻译缺陷。
...
> 字体使用具名 `'Songti SC', 'Noto Serif SC', serif`。macOS 上 pretext 测量依赖真实字体，缺失时回退到固定 `COL_SPACING = 8.5` 栅格。
...
- 凤冠 PNG 资源与文案 fork 自 [`aigc17/Chinese-PhoenixCrown`](https://github.com/aigc17/Chinese-PhoenixCrown)，按原项目许可使用（详见 [`LICENSE`](./LICENSE)）。
- 本仓库的布局引擎（pretext 测宽）、verlet 物理与交互为独立重构实现。
- 安全相关反馈见 [`SECURITY.md`](./SECURITY.md)。

## 自动 Changelog

每次 push 到 `main`，GitHub Actions（`.github/workflows/changelog.yml`）会将本次包含的 commits 追加进 `CHANGELOG.md`。
